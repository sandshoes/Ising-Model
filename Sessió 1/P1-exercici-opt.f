      program optexercise
      implicit none
      integer*4 SEED, i, NRAND, k(6), hist(6), DICE,dic
      real*8 genrand_real2, xlim, prob(6), x
      K=(/1,2,3,4,5,6/)
      hist = (/0,0,0,0,0,0/)
      PROB = (/0.083, 0.083, 0.083, 0.083, 0.083, 0.583/)
      SEED=23456
      NRAND=1000
      xlim = 0.0D0
      ...
      do i=1,NRAND
       x=genrand_real2()
       dic = DICE(x,PROB)
c       hist(dic) = hist(dic) +1 
      enddo
      end
      
      INTEGER*4 FUNCTION DICE(RND,PROB)
      integer*4 N, K(6),z
      real*8 PROB(6), rnd
      xlim=0.0D0
      do z=1,6
