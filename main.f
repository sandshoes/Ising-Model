...
C     Aquest programa simula el model d'Ising 2D de dimensió
C     L*L amb l'ajuda del mètode de Monte Carlo. Es busca
C     obtenir la magnetització, la susceptibilitat i la Cv en 
C     funció de la temperatura.
C
C     Tomás Healy Corominas, NIUB: 16677592
C     ------------------------------------------------------
      ...
      INTEGER*4 SEED,I,J,L,MCTOT,N,MCINI,MCD,NSEED,SEED0,TEMPERATURE
      PARAMETER (L=20)
      INTEGER*4 IMC,IPAS,SUMA,PBC(0:L+1)
      INTEGER*2 S(1:L,1:L)
      REAL*8 GENRAND_REAL2,MAG,MAGNE,TEMP,DE,DELTA,ENERG,ENE
      REAL*8 SUMS,SUME,SUME2,SUMM,SUMM2,SUMAM,VARE,VARM, DTEMP
      REAL*4 TIME1, TIME2
      CHARACTER*3 NOM

C     Inicialitzar parametres
...
C     Bucle de 200 calculs de temperatures
      DO TEMPERATURE = 1,200
        WRITE(*,*) 'PROCESSING TEMPERATURE ... ',TEMP
        SUMS = 0.0D0
        SUME = 0.0D0
        SUME2 = 0.0D0
        SUMM = 0.0D0
...
C       Creacio de la matriu de spins del sistema
          DO I = 1,L
            DO J = 1,L
              IF (GENRAND_REAL2().LT.0.5E0) THEN
                S(I,J)=1
              ELSE
                S(I,J)=-1
              ENDIF
            ENDDO
          ENDDO
C         Bucle de MCS i canvis a la matriu de spins
          DO IMC = 1, MCTOT
            DO IPAS = 1,N
              I = GENRAND_REAL2()*L +1
              J = GENRAND_REAL2()*L +1
...
C       Recoleccio de dades         
        ...
        WRITE(12,*) IMC,TEMP,SUMS,SUME/N,SUME2/N**2,VARE,SUMM/N,SUMAM/N,
     +              SUMM2/N**2,VARM
      ...
      OPEN(UNIT=15, FILE='TIME.OUT')
      WRITE(15,*) "CPU_TIME = ", TIME2-TIME1
      END PROGRAM FINAL
...
C     ---------------------------------------
      ...
      INTEGER*4 I,J,L
      INTEGER*2 S(1:L, 1:L)
      OPEN(UNIT=2,FILE='P1-configuration.conf')
      DO I =1,L
        DO J=1,L
          IF (S(I,J).EQ.1) THEN
            WRITE(2,100) I,J
